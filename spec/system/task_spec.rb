...
RSpec.describe 'タスク管理機能', type: :system do
  ...
  describe 'タスク管理機能', type: :system do
    describe 'ラベル機能' do
      ...
      context '編集機能' do
        it "生成したタスクのラベルを編集できる" do
          visit tasks_path
          visit edit_task_path(@task.id)
          uncheck "task[label_ids][]"
          click_on '更新する'
          visit tasks_path
          visit task_path(@task.id)
          expect(page).not_to have_content 'カテゴリー名'
        end
      end
      context '検索機能' do
        it "生成したタスクのラベルを検索できる" do
          visit new_task_path
          fill_in 'task[name]', with: "テストユーザー"
          fill_in 'task[content]', with: "test"
          click_on '登録する'
          visit tasks_path
          select 'カテゴリー名', from: 'label_id'
          expect(page).to have_content 'ユーザー１'
